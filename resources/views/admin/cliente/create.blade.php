    ...
    <table class="table table-striped" id = "cliente">
        <thead>
            <form action="{{route('admin.cliente.store')}}" method="POST">
                ...
                <br>
                <div class="row">
                    <label from="detalle" class="col-sm-2 col-form-label">{{('CI: ')}}</label> 
                    <div class="col-sm-7">
                    <input type="text" class="form-control " name="detalle" placeholder="Ingrese su Carnet de Identidad"> 
                    @if($errors->has('detalle'))
                        <span class="error text-darger" for="input-detalle">{{ $errors->first('detalle')}}</span>
                    @endif
                    </div>
                </div>
                <br>
